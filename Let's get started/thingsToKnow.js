...
Ans. The react.development.js is used for the debugging and showing detailed error messages for the smooth development environment. File size is larger than production and the redabilty of the code is better in development.

Whereas react.production.js is basically a minified/compressed version of the project. Its file size is much less than development but the redability is not developer friendly.


Q8. What is async and defer attributes in script tag?
Ans. async: In this attribute while the HTML is parsing the script are fetched along with it. After the script are fetched just after it the parsing stops and the scripts are executed and after the execution the parsing again starts.

defer: In this attrbute the HTML paring goes on and along with it the scripts are fetched but the exection of the scripts are only done when the parsing gets completed. 
*/
